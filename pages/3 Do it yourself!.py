...
try:
    ...
    def neuronal_network(excel_file_train_data, excel_file_test_data, layers=0, neurons=0):
        ...
        if (countZero > countOne):
            bb.ballons_blue()
            st.markdown("<h3 style='text-align: center;'>Die Person auf der Aufnahme scheint ein Mann zu sein!</h3>",
                        unsafe_allow_html=True)
        elif (countOne > countZero):
            br.ballons_red()
            st.markdown("<h3 style='text-align: center;'>Die Person auf der Aufnahme scheint eine Frau zu sein!</h3>",
                        unsafe_allow_html=True)
...
try:
    if upload_and_convert_newPath("tempDir2") == True and check2 == True:
        ...
        for i in range(num_layers):
            ...
            n = st.number_input(f"Wähle die Anzahl der Neuronen in der Schicht {i + 1}", min_value=1,
                                key=f'neurons_{i}')
            neurons.append(n)
        ...
        if st.button('Start'):
            for file in os.listdir("tempDir2/"):
                if file.endswith(".wav"):
                    excelFile = vc.get_single_excel_with_features_no_label(f"tempDir2/{file}", f"tempDir2/{file}", 10,
                                                                           True)
                    val_acc = neuronal_network("TrainDataFuerNeuronalesNetzohneGroupID.xlsx", excelFile, num_layers,
                                               neurons)
                    os.remove(f"{excelFile}")
                    val_acc = val_acc[len(val_acc) - 1]
                    st.markdown(
                        f"<h3 style='text-align: center;'>Die Validierungsgenauigkeit deines Modells entspricht: {val_acc[len(val_acc) - 1]}, diese kann stimmen muss sie aber nicht, denn abhängig von Tonqualität, Tonstärken der Audiodatei und Tonklang der Person kann die Vorhersage dennoch Falsch sein!</h3>",
                        unsafe_allow_html=True)
                    st.balloons()
                    os.remove(f"tempDir2/{file}")
                    st.markdown(
                        f"<div style='text-align: center;'>{get_binary_file_downloader_html(model_file_path, 'Dein-NN-Model.h5')}</div>",
                        unsafe_allow_html=True)
